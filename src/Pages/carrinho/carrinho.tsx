...
export default function Carrinho() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const listaCarrinho = JSON.parse(
          localStorage.getItem("carrinho") || "[]"
        );
        //Ele passará por todos os numeros da lista que possui id de livros do carrinho e verá qual
        // dos livros que tem esse id para por na lista de livro do carrinho do usuário
        const livrosNoCarrinho = listaCarrinho
          .map((id: number) =>
            response.data.find((livro: livro) => livro.id === id)
          )
  ...
  function Deletar(id: number) {
    ...
    let retirado = false;
    //precisei de duas lista, uma para atualizar o localstorage que armazena id
    // e a lista da propria página que usa instancias de livros
    let novaLista: livro[] = [];
    let novaListaId: number[] = [];
    if (confirmar) {
      livrosCar.map((livro: livro) => {
